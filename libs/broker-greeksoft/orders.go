...
func (c *Client) PlaceOrder(ctx context.Context, intent *broker.OrderIntent) (*broker.OrderResponse, error) {
	...
	if strings.Contains(responseText, `"rejectedBy":"IrisServer"`) ||
		strings.Contains(responseText, `"streaming_type":"IrisRejection"`) ||
		strings.Contains(responseText, `"svcName":"IrisRejection"`) {
		log.Printf("[GREEKSOFT ORDER] IRIS rejected raw=%s", responseText)
		return nil, fmt.Errorf("greeksoft IRIS rejected order: %s", responseText)
	}
