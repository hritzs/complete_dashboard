...
func (s *Store) GetContractToken(ctx context.Context, symbol, expiry, optionType string, strike float64) (int64, error) {
	exp, err := parseExpiry(expiry)
	if err != nil {
		return 0, err
	}
	...
	query := `
		...
		WHERE UPPER(symbol) = UPPER($1)
		  AND expiry_date::date = $2::date
		  AND UPPER(option_type) = UPPER($3)
		  AND strike_price = $4
		LIMIT 1
	`

	var token int64
	err = s.db.QueryRowContext(ctx, query, strings.TrimSpace(symbol), exp.Format("2006-01-02"), strings.TrimSpace(optionType), strike).Scan(&token)
	if err != nil {
		if err == sql.ErrNoRows {
			return 0, fmt.Errorf("contract token not found for %s %s %s %.0f",
				strings.ToUpper(strings.TrimSpace(symbol)),
				exp.Format("2006-01-02"),
				strings.ToUpper(strings.TrimSpace(optionType)),
				strike)
		}
		return 0, err
	}
